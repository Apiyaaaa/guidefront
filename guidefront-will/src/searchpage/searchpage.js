...
function Searchpage() {
  ...
  const getArticle = (word) => {
    axios({
      method: "GET",
      url: "http://127.0.0.1:5000/api/search_article",
      params: {
        word: word,
      },
    }).then((res) => {
...
const [sortType, setSortType] = useState('Newest');
  ...
  useEffect(() => {
    const sortArray = type => {
      ...
      if(sortProperty == 'MostViews'){
        setData([...data].sort((a, b) => b[sortProperty] - a[sortProperty]));
      } else if (sortProperty == 'Newest'){
        setData([...data].sort((a, b) => b[sortProperty].localeCompare(a[sortProperty])));
      }
